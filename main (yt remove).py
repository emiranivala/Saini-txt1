...
async def start(client: Client, msg: Message):
    ...
    await asyncio.sleep(1)
    await start_message.edit_text(Data.START.format(msg.from_user.mention) + "Checking status Ok... \nFor command **ᴊᴏɪɴ ᴏᴜʀ <a href='https://t.me/+taZCy5OkoMUwNzI9'>ᴛᴇʟᴇɢʀᴀᴍ Group</a>**\n\n**Bot Made BY 🇳‌🇮‌ᴊ‌ᴏʟ‍**🔍\n\nProgress:🟩🟩🟩🟩🟩🟩🟩🟩🟩 100%\n\n")
@bot.on_message(filters.command(["stop"]))
async def restart_handler(_, m):
    await m.reply_text("🦅ˢᵗᵒᵖᵖᵉᵈ ᵇᵃᵇʸ💞", True)
    os.execl(sys.executable, sys.executable, *sys.argv)
@bot.on_message(filters.command(["crushelover"]))
